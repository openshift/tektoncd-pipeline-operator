...
function get_version {
    ...
    local version=$(python -c "from pkg_resources import parse_version;import sys, json, requests;jeez=requests.get('https://api.github.com/repos/tektoncd/pipeline/releases').json();print(sorted([x['tag_name'] for x in jeez], key=parse_version, reverse=True)[${shift}])")
